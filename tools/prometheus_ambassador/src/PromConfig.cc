...
PromConfig::PromConfig(const nlohmann::json& config_json)
    : prometheus_address(
          config_json.value(PA_CONFIG_PROMETHEUS_ADDRESS_KEY, PA_CONFIG_PROMETHEUS_ADDRESS_DEFAULT))
    , prometheus_port(
          config_json.value(PA_CONFIG_PROMETHEUS_PORT_KEY, PA_CONFIG_PROMETHEUS_PORT_DEFAULT))
    ...
    , protocols(
          parse_protocol(config_json.value(PA_CONFIG_PROTOCOLS_KEY, PA_CONFIG_PROTOCOLS_DEFAULT)))
