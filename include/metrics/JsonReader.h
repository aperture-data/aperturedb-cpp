...
struct ParseAs< double > {
    ...
    double operator()(const JSON& json)
    {
        ...
        if (json.is_string()) return std::stod(json.template get_ref< const std::string& >());
    ...
    prometheus::MetricType operator()(const JSON& json)
    {
        if (json.is_string()) {
            switch (json.template get_ref< const std::string& >()[0]) {
                ...
                case 'S':
                    return prometheus::MetricType::Summary;
                case 'U':
...
class JsonReader
{
   ...
   private:
    ...
    prometheus::ClientMetric& emplace_metric_metadata(prometheus::MetricFamily& fam,
                                                      const json_type& mtc)
    {
    ...
    void parse_family_values(prometheus::MetricFamily& fam, const json_type& json)
    {
        switch (fam.type) {
            case prometheus::MetricType::Counter:
                return parse_counter_values(fam, json);
            case prometheus::MetricType::Gauge:
                return parse_gauge_values(fam, json);
            case prometheus::MetricType::Summary:
                return parse_summary_values(fam, json);
            case prometheus::MetricType::Untyped:
                return parse_untyped_values(fam, json);
            case prometheus::MetricType::Histogram:
                return parse_histogram_values(fam, json);
        }
    }
