...
class JsonWriter
{
   ...
   public:
    ...
    json_type to_json(int64_t val) { return json_type(val); }

    json_type to_json(uint64_t val) { return json_type(val); }
    ...
    json_type to_json(prometheus::MetricType type)
    {
        switch (type) {
            case prometheus::MetricType::Counter:
                return json_type("Counter");
            case prometheus::MetricType::Gauge:
                return json_type("Gauge");
            case prometheus::MetricType::Summary:
                return json_type("Summary");
            case prometheus::MetricType::Untyped:
                return json_type("Untyped");
            case prometheus::MetricType::Histogram:
                return json_type("Histogram");
        };
    }
    ENABLE_WARNING(return-type)

    json_type to_json(const std::string& val) { return json_type(val); }
    ...
    json_type to_json(const prometheus::ClientMetric& metric, prometheus::MetricType type)
    {
        json_type val;
