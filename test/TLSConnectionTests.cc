...
class TLSConnectionTests : public testing::Test
{
   protected:
    void SetUp() override
    {
        ...
        connServerConfig = comm::TCPConnServerConfig( comm::ConnServerConfig {
...
#define copyWithTcpPort(port,config) comm::createConnList( comm::wrapConnServerConfig( &(new comm::TCPConnServerConfig( config ))->addPort(port)) )
TEST_F(TLSConnectionTests, ServerWrongPort)
{
    ASSERT_THROW(comm::ConnServer server(copyWithTcpPort(-22, connServerConfig)), comm::Exception);

    ASSERT_THROW(comm::ConnServer server(copyWithTcpPort(0, connServerConfig)), comm::Exception);
}
...
TEST_F(TLSConnectionTests, UseAutogeneratedCertificates)
{
    ...
    std::thread server_thread([&]() {
        static const comm::TCPConnServerConfig connServerConfig_(SERVER_PORT_INTERCHANGE, comm::ConnServerConfig {comm::Protocol::TLS, true});

        comm::ConnServer server(comm::createConnList( comm::wrapConnServerConfig( connServerConfig_)));
