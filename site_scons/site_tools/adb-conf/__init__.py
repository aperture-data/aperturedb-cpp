...
def adbAddPackageFromOption(env,package_name,option_name,env_prefix,with_lib=True,override={}):
    env.adbData.addPackageWithOption(env,package_name,option_name,env_prefix,with_lib,override)

def adbAddLocalPackage(env,package_name,env_prefix,inc_path,lib_path,override={}):
    env.adbData.addLocalPackage(env,package_name,env_prefix,inc_path,lib_path,override)
...
def generate(env):
    ...
    class ApertureDataConf():
        ...
        def getPackageIncludePath(self,pkg):
            ...
            if not "include" in package:
                err=f"Pacakge {pkg} has no defined includes"
                raise BaseException(err)
            if "root" in package:
                current_path = env.Dir('.').srcnode().get_abspath()
                relative_include = os.path.relpath( os.path.join(package["root"],package["include"]),current_path)
        ...
        def getPackageLibPaths(self,*packages):
            ...
            for pkg in packages:
                if not self.hasPackage(pkg):
                    ...
                    raise BaseException(err)
                package = self.packageList[pkg]
                if not "lib" in package:
                    err=f"Pacakge {pkg} has no defined library"
                    raise BaseException(err)
                lib_path = None
                if "root" in package:
                    current_path = env.Dir('.').srcnode().get_abspath()
                    relative_include = os.path.relpath( os.path.join(package["root"],package["lib"]),current_path)
        ...
        def addLocalPackage(self,env,package_name,env_prefix,inc_path,lib_path,override):
            if self.hasPackage(package_name):
                raise BaseException("Cannot add Package " + package_name +": Already exists")
        ...
        def addPackageWithOption(self,env,package_name,cmd_option,env_prefix,with_lib,override):
            if self.hasPackage(package_name):
                raise BaseException("Cannot add Package " + package_name +": Already exists")

            if not "ENV_INC_POST" in override:
            ...
            if cmd_option in self.optionMap:
                option_inc_path = os.path.join(env.GetOption(cmd_option), 'include')
            else:
                option_inc_path = os.getenv(env_include, default=os.path.join(env.GetOption(cmd_option),'include'))
            package["include"] = option_inc_path
            if with_lib:
                env_include = env_prefix + "_LIB"
                option_lib_path = ""
                if cmd_option in self.optionMap:
                    option_lib_path = os.path.join(env.GetOption(cmd_option), 'lib')
                else:
                    option_lib_path = os.getenv(env_include, default=os.path.join(env.GetOption(cmd_option),'lib'))
